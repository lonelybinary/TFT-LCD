 ...
 * 使用 Arduino GFX 官方库
 * 驱动芯片：ST7735S
 * 屏幕型号：N096-1608TBBIG11-H13 (Newvision)
 * 分辨率：160x80 
 */
...
// ST7735S 初始化参数：160x80
// 偏移量说明：
// - col offset: 列偏移（左右移动），通常 26-28
// - row offset: 行偏移（上下移动），通常 1-3
// - offset 1 用于 rotation 0/2，offset 2 用于 rotation 1/3
Arduino_DataBus *bus = new Arduino_SWSPI(TFT_DC, TFT_CS, TFT_SCLK, TFT_MOSI, -1);
Arduino_ST7735 *gfx = new Arduino_ST7735(bus, TFT_RST, 0 /* rotation */, true /* IPS */, 80 /* width */, 160 /* height */, 24 /* col offset 1 */, 0 /* row offset 1 */, 24 /* col offset 2 */, 0 /* row offset 2 */);
...
void testText() {
  ...
  gfx->setTextSize(1);
  gfx->println("RESOLUTION:160x80");
  gfx->println("DRIVER IC:ST7735S");
...
// ==================== 偏移量校准测试 ====================
// 用于检查显示位置是否准确
void testOffsetCalibration() {
  Serial.println("测试: 偏移量校准");
  ...
  // 四角标记
  gfx->fillRect(0, 0, 5, 5, RED);                                    // 左上角 - 红色
  gfx->fillRect(gfx->width() - 5, 0, 5, 5, GREEN);                   // 右上角 - 绿色
  gfx->fillRect(0, gfx->height() - 5, 5, 5, BLUE);                    // 左下角 - 蓝色
  gfx->fillRect(gfx->width() - 5, gfx->height() - 5, 5, 5, YELLOW);  // 右下角 - 黄色
  
  // 中心十字线
  gfx->drawLine(gfx->width() / 2, 0, gfx->width() / 2, gfx->height(), CYAN);
  gfx->drawLine(0, gfx->height() / 2, gfx->width(), gfx->height() / 2, CYAN);
  
  // 显示当前偏移量
  gfx->setCursor(10, 10);
  gfx->setTextColor(WHITE);
  gfx->setTextSize(1);
  gfx->println("Offset Test");
  gfx->print("Col: 26, Row: 1");
...
void setup() {
  ...
  Serial.println("驱动芯片: ST7735S");
  Serial.println("屏幕型号: N096-1608TBBIG11-H13");
  Serial.println("分辨率: 160x80");
  Serial.println("使用 Arduino GFX 官方库");
  Serial.println("========================================");
  
  // 初始化背光（低电平有效，直接设置为LOW）
  pinMode(TFT_BACKLIGHT, OUTPUT);
  digitalWrite(TFT_BACKLIGHT, LOW);  // 低电平亮
  ...
  // ST7735S 初始化
  Serial.println("初始化 ST7735S 屏幕 (160x80)...");
