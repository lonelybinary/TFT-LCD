 ...
 * SPI接口 LCD 测试程序
 * 使用 Arduino GFX 官方库
 * 驱动芯片：ST7365（规格书）/ ST7796（兼容驱动实现）
 * 屏幕尺寸：3.5 英寸
 * 分辨率：320x480（竖屏原生），可通过 setRotation() 变为 480x320
 */
...
// ST7796（兼容 ST7365）原生分辨率：320x480
...
void setup() {
  ...
  Serial.println("驱动芯片: ST7365");
  Serial.println("屏幕尺寸: 3.5 英寸");
  Serial.println("分辨率: 320x480");
  ...
  // ST7365（兼容 ST7796）初始化：原生 320x480
  Serial.println("初始化 ST7365 屏幕 (320x480)...");
  ...
  // 设置旋转方向
  gfx->setRotation(0);  // 0=竖屏, 1/3=横屏, 2=竖屏倒置
