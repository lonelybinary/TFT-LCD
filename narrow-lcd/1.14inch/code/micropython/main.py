...
Display:    1.14 inch, 135x240 pixels (240x135 in landscape)
Driver IC:  ST7789
...
2. Copy the driver and this script onto the board:
     ...
     mpremote cp narrow-lcd/1.14inch/code/micropython/main.py :
...
def display_lonely_binary(tft):
    ...
    # Top section: the title, centered across the 240-pixel landscape width.
    title = "LonelyBinary"
    text_x = (tft.width - len(title) * 8 * 2) // 2
    tft.text(title, text_x, 4, st77xx.WHITE, scale=2)
    ...
    info_y = 103
    tft.text("240x135", 4, info_y, st77xx.WHITE)
    tft.text("ST7789", 4, info_y + 10, st77xx.WHITE)
    tft.text("1.14inch", 4, info_y + 20, st77xx.WHITE)
...
print("Display Size: 1.14 inch")
print("Resolution: 135x240 (240x135 landscape)")
...
# rotation=1 puts the long edge across (240x135); this panel is only 135 px
...
# horizontal one. The values below are the Arduino sketch's col offset 2 (53)
# and row offset 1 (40) respectively - the two that are active at rotation 1.
tft = st77xx.ST7789(spi, 135, 240,
                    dc=Pin(TFT_DC), cs=Pin(TFT_CS), rst=Pin(TFT_RST),
                    rotation=1, xstart=53, ystart=40,
