...
// Then "gfx" is the actual ST7789 screen driver we call to draw.
//
// ROTATION 1 (landscape). Every screen in this collection is a narrow strip:
// this one is 135x240 as wired, which is only 135 pixels across - too narrow for
// most text. Rotation 1 turns it on its side and gives us 240x135. The wider
...
Arduino_ST7789 *gfx = new Arduino_ST7789(bus, TFT_RST, 1 /* rotation */, true /* IPS */, 135 /* width */, 240 /* height */,
...
void setup() {
  ...
  Serial.println("Display Size: 1.14 inch");
  Serial.println("Resolution: 135x240 (240x135 landscape)");
  Serial.println("----------------------------------------");
...
void showInfo() {
  ...
  gfx->setTextColor(WHITE);
  gfx->setCursor(4, y); gfx->print("1.14in ST7789"); y += lineH;
  gfx->setCursor(4, y); gfx->printf("%dx%d RGB565", gfx->width(), gfx->height()); y += lineH;
  gfx->setCursor(4, y); gfx->print("SPI 8MHz IPS on"); y += lineH;
