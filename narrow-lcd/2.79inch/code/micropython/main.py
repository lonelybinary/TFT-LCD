"""
SPI TFT-LCD Test Script (MicroPython) - Narrow TFT LCD Collection
...
2. Copy the drivers and this script onto the board:
     ...
     mpremote cp narrow-lcd/2.79inch/code/micropython/main.py :
...
def display_lonely_binary(tft):
    ...
    # Top section: the title, centered across the 428-pixel landscape width.
    title = "LonelyBinary"
    text_x = (tft.width - len(title) * 8 * 3) // 2
    tft.text(title, text_x, 4, st77xx.WHITE, scale=3)
    ...
    info_y = 108
    tft.text("428x142", 4, info_y, st77xx.WHITE)
    tft.text("NV3007", 4, info_y + 10, st77xx.WHITE)
    tft.text("2.79inch", 4, info_y + 20, st77xx.WHITE)


# ==================== Initialization ====================
print("========================================")
print("SPI LCD Test Program (MicroPython)")
print("Narrow TFT LCD Collection")
print("Driver IC: NV3007")
print("Display Size: 2.79 inch")
print("Resolution: 142x428 (428x142 landscape)")
print("Board detected:", "ESP32-S3" if IS_S3 else "classic ESP32")
print("========================================")

# Turn the backlight on. On the narrow modules the backlight is active-low:
# LOW = ON (several of the wider sizes in this kit use HIGH = ON).
backlight = Pin(TFT_BACKLIGHT, Pin.OUT, value=0)

# Hardware SPI. Only MOSI and SCLK are wired - the display has no MISO.
# 20 MHz is the clock this panel was tested at with the Arduino sketch.
spi = SPI(SPI_ID, baudrate=20000000, polarity=0, phase=0,
          sck=Pin(TFT_SCLK), mosi=Pin(TFT_MOSI), miso=None)

print("Initializing LCD...")
# rotation=1 puts the long edge across (428x142); this panel is only 142 px
# wide as wired, which is too narrow for the title text.
#
# Offsets: this driver keeps ONE x/y offset pair and swaps them for landscape,
# so at rotation 1 `xstart` becomes the vertical offset and `ystart` the
# horizontal one. The values below are the Arduino sketch's col offset 2 (14)
# and row offset 1 (0) respectively - the two that are active at rotation 1.
# (The visible glass sits at columns 12-153 of the chip's 168-column RAM, and
# rotation 1 counts from the far end: 167 - 153 = 14.)
#
# NV3007_279 carries the 2.79 inch vendor init table - same chip as the
# 1.68 inch but different voltage/gamma values, so it must not be swapped.
tft = nv3007.NV3007_279(spi, dc=Pin(TFT_DC), cs=Pin(TFT_CS), rst=Pin(TFT_RST),
