...
- **SPI clock:** 20 MHz (tested)
- **Power:** 3.3V only (never 5V)
- **Touch:** no — this is a display-only module

> **One thing to know about this size:** Two things are special. The driver is an **NV3007**, and it needs its own initialisation table (`nv3007_279_init_operations`) - the same chip as the 1.68 inch but different voltage/gamma settings, so the table is passed explicitly in the sketch. It also needs `col offset 2 = 14` while `col offset 1 = 12`, for the same reason as the 1.68 inch. Measured on hardware.
...
1. Open `code/2.79inch_Test/2.79inch_Test.ino` in the Arduino IDE.
