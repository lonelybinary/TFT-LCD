# 2.25 Inch TFT-LCD Display (Narrow TFT LCD Collection)

A 2.25-inch narrow color display for your ESP32 projects — the thinnest strip in the collection - only 76 pixels tall in landscape. This page covers the
specs and wiring for this size. For full setup steps, see the [main README](../README.md).

## The basics

- **Driver chip:** ST7789P3
- **Resolution:** 76 × 284 pixels (portrait, as wired)
- **In the examples:** 284 × 76 — the examples run in landscape (rotation 1)
...
> **One thing to know about this size:** The IPS setting must stay **`false`** on this panel. With `true` every colour comes out bit-inverted (black shows as white, red as cyan). The example already has this right.

## Why landscape?

Every screen in this collection is a narrow strip. Held the way it is wired, this one is only
**76 pixels wide** — not enough room for a word like "LonelyBinary" across the top. So the
examples call `setRotation(1)`, which turns the panel on its side and gives you **284 × 76**.
...
This size ships with `82, 18, 82, 18` (col offset 1, row offset 1, col offset 2, row offset 2).
...
So on the examples in this folder, the two numbers that matter are **row offset 1 = 18**
(horizontal) and **col offset 2 = 82** (vertical). Editing col offset 1 will appear to do
nothing at all.
...
1. Open `code/2.25inch_Test/2.25inch_Test.ino` in the Arduino IDE.
...
`ADAPTATION_GUIDE.md` to adjust them for the 2.25-inch screen.
