...
// Then "gfx" is the actual ST7789P3 screen driver we call to draw.
//
// ROTATION 1 (landscape). Every screen in this collection is a narrow strip:
// this one is 76x284 as wired, which is only 76 pixels across - too narrow for
// most text. Rotation 1 turns it on its side and gives us 284x76. The wider
...
Arduino_ST7789 *gfx = new Arduino_ST7789(bus, TFT_RST, 1 /* rotation */, false /* IPS */, 76 /* width */, 284 /* height */,
    82 /* col offset 1 */, 18 /* row offset 1 */, 82 /* col offset 2 */, 18 /* row offset 2 */);
...
void setup() {
  ...
  // 8 MHz is the SPI clock this panel was tested at.
  ...
  Serial.println("Driver IC: ST7789P3");
  Serial.println("Display Size: 2.25 inch");
  Serial.println("Resolution: 76x284 (284x76 landscape)");
  Serial.println("----------------------------------------");
...
void showInfo() {
  ...
  gfx->setTextColor(WHITE);
  gfx->setCursor(4, y); gfx->print("2.25in ST7789P3"); y += lineH;
  gfx->setCursor(4, y); gfx->printf("%dx%d RGB565", gfx->width(), gfx->height()); y += lineH;
  gfx->setCursor(4, y); gfx->print("SPI 8MHz IPS off"); y += lineH;
