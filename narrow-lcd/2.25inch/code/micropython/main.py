...
2. Copy the driver and this script onto the board:
     ...
     mpremote cp 2.25inch/code/micropython/main.py :
...
def display_lonely_binary(tft):
    ...
    # Top section: the title, centered across the 284-pixel landscape width.
    title = "LonelyBinary"
    ...
    # Only 76 pixels tall in landscape - one combined line instead of three.
    info_y = 64
    tft.text("284x76 ST7789P3 2.25inch", 4, info_y, st77xx.WHITE)
...
print("Driver IC: ST7789P3")
print("Display Size: 2.25 inch")
print("Resolution: 76x284 (284x76 landscape)")
...
# rotation=1 puts the long edge across (284x76); this panel is only 76 px
...
# horizontal one. The values below are the Arduino sketch's col offset 2 (82)
# and row offset 1 (18) respectively - the two that are active at rotation 1.
tft = st77xx.ST7789(spi, 76, 284,
                    dc=Pin(TFT_DC), cs=Pin(TFT_CS), rst=Pin(TFT_RST),
                    rotation=1, xstart=82, ystart=18,
                    bgr=False, invert=False)
