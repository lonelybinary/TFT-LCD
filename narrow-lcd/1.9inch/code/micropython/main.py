...
Display:    1.9 inch, 170x320 pixels (320x170 in landscape)
Driver IC:  ST7789
...
2. Copy the driver and this script onto the board:
     ...
     mpremote cp narrow-lcd/1.9inch/code/micropython/main.py :
...
def display_lonely_binary(tft):
    ...
    # Top section: the title, centered across the 320-pixel landscape width.
    title = "LonelyBinary"
    ...
    info_y = 138
    tft.text("320x170", 4, info_y, st77xx.WHITE)
    tft.text("ST7789", 4, info_y + 10, st77xx.WHITE)
    tft.text("1.9inch", 4, info_y + 20, st77xx.WHITE)
...
print("Display Size: 1.9 inch")
print("Resolution: 170x320 (320x170 landscape)")
...
# rotation=1 puts the long edge across (320x170); this panel is only 170 px
...
# horizontal one. The values below are the Arduino sketch's col offset 2 (35)
# and row offset 1 (0) respectively - the two that are active at rotation 1.
tft = st77xx.ST7789(spi, 170, 320,
                    dc=Pin(TFT_DC), cs=Pin(TFT_CS), rst=Pin(TFT_RST),
                    rotation=1, xstart=35, ystart=0,
