# 1.68 Inch TFT-LCD Display (Narrow TFT LCD Collection)

A 1.68-inch narrow color display for your ESP32 projects — a long, narrow strip - the same panel as the 2.79 inch at a finer pixel pitch. This page covers the
specs and wiring for this size. For full setup steps, see the [main README](../README.md).

## The basics

- **Driver chip:** NV3007
- **Resolution:** 142 × 428 pixels (portrait, as wired)
- **In the examples:** 428 × 142 — the examples run in landscape (rotation 1)
...
> **One thing to know about this size:** The driver is an **NV3007**, not an ST7789. It also needs `col offset 2 = 14` while `col offset 1 = 12`: the visible area sits at columns 12-153 of the controller's 168-column RAM, which is not centred, and rotation 1 counts that offset from the far end (167 - 153 = 14). Measured on hardware.

## Why landscape?

Every screen in this collection is a narrow strip. Held the way it is wired, this one is only
**142 pixels wide** — not enough room for a word like "LonelyBinary" across the top. So the
examples call `setRotation(1)`, which turns the panel on its side and gives you **428 × 142**.
...
This size ships with `12, 0, 14, 0` (col offset 1, row offset 1, col offset 2, row offset 2).
...
(horizontal) and **col offset 2 = 14** (vertical). Editing col offset 1 will appear to do
nothing at all.
...
1. Open `code/1.68inch_Test/1.68inch_Test.ino` in the Arduino IDE.
...
`ADAPTATION_GUIDE.md` to adjust them for the 1.68-inch screen.
...
the script. The `NV3007_168` class carries this size's own init table.
...
- That is the IPS setting. This size uses `false`; the example already has it right.
