...
Display:    1.8 inch, 128x160 pixels
Driver IC:  ST7735S
...
2. Copy the driver and this script onto the board:
     ...
     mpremote cp tft-lcd/1.8inch/code/micropython/main.py :
...
def display_lonely_binary(tft):
    ...
    # so at scale 1 the title is 12 * 8 = 96 px wide - fits 128 px.
    title = "LonelyBinary"
    text_x = (tft.width - len(title) * 8) // 2
    tft.text(title, text_x, 24, st77xx.WHITE)

    # Middle section: 8 horizontal color bars.
    # (The Arduino sketch sets this panel to BGR mode and then swaps the
    # colors back in software; here we simply leave the panel in RGB mode.)
    ...
    info_y = 135
    tft.text("128x160", 2, info_y, st77xx.WHITE)
    tft.text("ST7735S", 2, info_y + 8, st77xx.WHITE)
    tft.text("1.8inch", 2, info_y + 16, st77xx.WHITE)
