# TFT-LCD Series (0.96 – 3.5 inch)

Six IPS colour displays for your ESP32 projects, from a tiny 0.96-inch strip to a 3.5-inch
panel — the classic sizes for dashboards, gauges, menus and games. Every module uses the
**same 15-pin FPC connector** as every other Lonely Binary display, so they all plug into the
same breakout board: swap sizes (or move to the [Narrow LCD series](../narrow-lcd/README.md))
...
New to these displays? Read this page top to bottom, then follow the step-by-step lessons in
...
| Size      | Resolution | Driver chip | Folder |
| --------- | ---------- | ----------- | ------ |
| 0.96 inch | 80 × 160   | ST7735S     | [`0.96inch/`](0.96inch/README.md) |
| 1.8 inch  | 128 × 160  | ST7735S     | [`1.8inch/`](1.8inch/README.md) |
| 2.0 inch  | 240 × 320  | ST7789      | [`2.0inch/`](2.0inch/README.md) |
| 2.4 inch  | 240 × 320  | ST7789      | [`2.4inch/`](2.4inch/README.md) |
| 2.8 inch  | 240 × 320  | ST7789      | [`2.8inch/`](2.8inch/README.md) |
| 3.5 inch  | 320 × 480  | ST7796      | [`3.5inch/`](3.5inch/README.md) |
...
MicroPython firmware on both boards, using the shared driver
[`micropython/lib/st77xx.py`](../micropython/lib/st77xx.py) at the **root of the repository**
(not inside the size folder — the `README.md` next to each `main.py` shows exactly which files
to copy). Setup guide: [micropython/README.md](../micropython/README.md). There is also a full
...
- **Backlight** is simple on/off (no brightness/PWM control). On every size in this series
  `HIGH` = on, **except the 0.96 inch, where `LOW` = on** — the sketches already handle this.
- **Rotation:** the examples use rotation 0 (portrait, as wired).
