...
Display:    3.5 inch, 320x480 pixels (portrait; use tft.set_rotation(1) for 480x320)
Driver IC:  ST7796
...
2. Copy the driver and this script onto the board:
     ...
     mpremote cp tft-lcd/3.5inch/code/micropython/main.py :
...
def display_lonely_binary(tft):
    ...
    # so at scale 2 the title is 12 * 8 * 2 = 192 px wide - fits 320 px.
    title = "LonelyBinary"
    scale = 2
    text_x = (tft.width - len(title) * 8 * scale) // 2
    text_y = tft.height // 4 - 6  # top section middle
    tft.text(title, text_x, text_y, st77xx.WHITE, scale=scale)
    ...
    bar_start_y = tft.height // 2 - 6
    bar_height = (tft.height // 2 - 50) // 8
    ...
    info_y = tft.height - 46
    tft.text("320x480", 10, info_y, st77xx.WHITE)
    tft.text("ST7796", 10, info_y + 12, st77xx.WHITE)
    tft.text("3.5inch", 10, info_y + 24, st77xx.WHITE)


# ==================== Initialization ====================
print("========================================")
print("SPI LCD Test Program (MicroPython)")
print("Driver IC: ST7796")
print("Display Size: 3.5 inch")
print("Resolution: 320x480")
...
# bgr=True: this ST7796 panel wants BGR color order (red/blue would be
# swapped without it).
tft = st77xx.ST7796(spi, 320, 480,
